...
- [x] (NEW!) Any custom model support (Gemini, Claude, local models) without OpenAI model restrictions
- [x] (NEW!) Automatic `FALLBACK_MODEL` failover on API rate limits or errors
- [x] (NEW!) Smart group chat pinging for text, voice messages, video notes, and photos
- [x] (NEW!) Free Flux image generation via Pollinations.ai with `/image`, `/photo`, and natural language prompts
- [x] (NEW!) Telegram command popup autosuggestions for group and private chats
...
| `FALLBACK_MODEL`                    | Fallback model used automatically when the primary model fails (e.g. rate limit, 404, 500)                                                                                                                                                                                               | -                                  |
| `OPENAI_BASE_URL`                   | Endpoint URL for unofficial OpenAI-compatible APIs (e.g., LiteLLM, OneAPI, LocalAI or text-generation-webui)                                                                                                                                                                             | Default OpenAI API URL             |
...
| `VISION_MODEL`                      | The Vision model to use (defaults to `OPENAI_MODEL` if unset)                                                                                                                                                                                                                           | `OPENAI_MODEL`                     |
...
| `IMAGE_MODEL`                       | Image generation model/backend. Available models: `pollinations` (free Flux), `dall-e-2` and `dall-e-3`                                                                                                                                                                                | `pollinations`                     |
| `POLLINATIONS_MODEL`                | Model name for Pollinations image generation. e.g. `flux`, `flux-realism`, `flux-anime`, `black-forest-labs/flux.2-klein-4b`                                                                                                                                                           | `flux`                             |
| `POLLINATIONS_API_KEY`              | Optional API key for Pollinations supporter tiers                                                                                                                                                                                                                                       | -                                  |
